...
public:
    ...
    void draw(const Renderer& renderer, TTF_Font* titleFont, TTF_Font* uiFont, const LocalizationManager& localization, const SettingsManager& settings) {
        ...
        renderer.drawText(localization.get("title"), centerX, centerY - 220.0f, titleColor, titleFont);

        SDL_Color subtitleColor{160, 180, 210, 255};
        renderer.drawText(localization.get("select_category"), centerX, centerY - 165.0f, subtitleColor, uiFont);

        float cardW = 420.0f;
        float cardH = 48.0f;
        float cardYStart = centerY - 120.0f;
        for (size_t i = 0; i < m_categories.size(); ++i) {
            float y = cardYStart + i * (cardH + 10.0f);
            float x = centerX - cardW / 2.0f;

            SDL_Color cardBg{28, 36, 48, 255};
            renderer.drawRoundedRect(x, y, cardW, cardH, 14.0f, cardBg);

            SDL_Color borderColor{55, 70, 100, 255};
            renderer.drawRoundedRect(x + 1.0f, y + 1.0f, cardW - 2.0f, cardH - 2.0f, 13.0f, borderColor);

            SDL_Color catColor{220, 230, 255, 255};
            std::string line = std::to_string(i + 1) + ".  " + m_categories[i];
            renderer.drawText(line, centerX, y + cardH / 2.0f, catColor, uiFont);
        }
        ...
        renderer.drawText(diffText, centerX, centerY + 100.0f, diffColor, uiFont);
        ...
        renderer.drawText(dailyText, centerX, centerY + 140.0f, dailyColor, uiFont);

        SDL_Color hintColor{100, 115, 145, 255};
        renderer.drawText(localization.get("select_category_hint"), centerX, centerY + 200.0f, hintColor, uiFont);
        renderer.drawText(localization.get("difficulty_hint"), centerX, centerY + 235.0f, hintColor, uiFont);
        renderer.drawText("[S] " + localization.get("settings_key"), centerX, centerY + 270.0f, hintColor, uiFont);
    }
